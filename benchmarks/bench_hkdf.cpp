// bench_hkdf.cpp — HKDF (RFC 5869) 多长度 × 多实现 × OpenSSL 对比微基准
//
// 覆盖: HKDF-SHA256 / HKDF-SHA384 / HKDF-SHA512
...
//
// 参数 (固定合理值):
//   salt 16B / info 16B; OKM 长度按变体 = 32B(sha256) / 48B(sha384) / 64B(sha512);
//   消息 (IKM) 长度为矩阵: 16 / 256 / 4096 / 65536 / 1048576 字节。
//
// 正确性自检: 每个变体在全部 5 档长度与 OpenSSL 逐字节比对 OKM,
//   任一 FAIL 立即非零退出 (自检始终全部档位, 不受 BENCH_SMOKE 影响)。
//
// 全量 vs smoke: 环境变量 BENCH_SMOKE=1 时基准只测 16/256 两档、
//   每轮约 80ms、1 轮; 未设置时 5 档、每轮约 150ms、3 轮取最小。
//
// 编译 (worktree 根):
//   g++ -O2 -std=c++20 -fopenmp -DJP_AVX2 -DJP_VAES -Iinclude -Isrc \
//       benchmarks/bench_hkdf.cpp \
//       /home/jp/jpssl/build-main-verify/libjpssl_cpu.a \
//       -lcrypto -o /tmp/bench_hkdf
//
// 输出: stdout 人类可读表格 + benchmarks/results/bench_hkdf.csv
//   CSV 列: algo,impl,size_bytes,ns_per_op,throughput_mbps
//   algo 取 hkdf-sha256 / hkdf-sha384 / hkdf-sha512; impl 取 jpssl / openssl
...
static void bench_case(const char* algo, const char* impl, size_t size,
    ...
    std::printf("%-13s %-9s %10zu %12.0f %12.1f\n",
...
static constexpr size_t kSizesAll[] = {16, 256, 4096, 65536, 1048576};
...
static const Variant kVariants[] = {
    {"hkdf-sha256", "SHA256", 32, jp_hkdf_sha256},
    {"hkdf-sha384", "SHA384", 48, jp_hkdf_sha384},
    {"hkdf-sha512", "SHA512", 64, jp_hkdf_sha512},
};
...
int main() {
    ...
    std::printf("=== jpssl HKDF (RFC 5869) vs OpenSSL 微基准 ===\n");
    std::printf("OpenSSL version: %s\n", OPENSSL_VERSION_TEXT);
    std::printf("BENCH_SMOKE     : %s (bench 档位=%zu, ~%.0fms/轮, %d 轮)\n",
                smoke ? "on" : "off", bench_n, target_ms, rounds);
    ...
    uint8_t salt[16], info[16];
    for (int i = 0; i < 16; ++i) salt[i] = (uint8_t)(0x40 + i);
    for (int i = 0; i < 16; ++i) info[i] = (uint8_t)(0x80 + i);

    // ── 正确性自检 (jpssl vs OpenSSL, 始终全部档位) ──
    std::printf("\n--- 正确性自检 (jpssl vs OpenSSL, 逐字节比对 OKM) ---\n");
    bool all_pass = true;
    size_t pass_cnt = 0;
    ...
    kd256.salt = salt; kd256.salt_len = 16; kd256.info = info; kd256.info_len = 16;
    kd384.salt = salt; kd384.salt_len = 16; kd384.info = info; kd384.info_len = 16;
    kd512.salt = salt; kd512.salt_len = 16; kd512.info = info; kd512.info_len = 16;
    ...
    for (size_t s : kSizesAll) {
        for (const Variant& v : kVariants) {
            OsslKdf* kd = (v.algo == std::string("hkdf-sha256")) ? &kd256
                         : (v.algo == std::string("hkdf-sha384")) ? &kd384 : &kd512;
            uint8_t o1[64], o2[64];
            v.jp(salt, 16, g_data, s, info, 16, o1, v.okm_len);
            if (!kd->ok || !kd->derive(g_data, s, o2, v.okm_len)) {
                char tag[80];
                std::snprintf(tag, sizeof(tag), "%s len=%zu (ossl kdf)", v.algo, s);
                check(tag, "kdf-error", "ok");
            } else {
                char tag[80];
                std::snprintf(tag, sizeof(tag), "%s len=%zu", v.algo, s);
    ...
    for (size_t i = 0; i < bench_n; ++i) {
        ...
        for (const Variant& v : kVariants) {
            OsslKdf* kd = (v.algo == std::string("hkdf-sha256")) ? &kd256
                         : (v.algo == std::string("hkdf-sha384")) ? &kd384 : &kd512;
            uint8_t o[64];
            bench_case(v.algo, "jpssl", s, (double)s, target_ms, rounds,
                       [&] { v.jp(salt, 16, g_data, s, info, 16, o, v.okm_len); g_sink ^= o[0]; });
            bench_case(v.algo, "openssl", s, (double)s, target_ms, rounds,
                       [&] { kd->derive(g_data, s, o, v.okm_len); g_sink ^= o[0]; });
        }
    }
    ...
    std::FILE* fp = std::fopen("benchmarks/results/bench_hkdf.csv", "w");
    if (!fp) {
    ...
    std::fclose(fp);
    std::printf("\nCSV 已写入 benchmarks/results/bench_hkdf.csv (%zu 数据行)\n", g_rows.size());
    return 0;
}
