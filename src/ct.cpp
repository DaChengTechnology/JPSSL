...
decode_sct_list_extn(const std::vector<uint8_t>& extn_value) {
    ...
    size_t lo = 0;
    uint16_t total;
    if (!read_u16(list, list_len, lo, total)) return std::nullopt;
    if ((size_t)total != list_len - 2) return std::nullopt;
    std::vector<signed_certificate_timestamp> scts;
    while (lo < list_len) {
        uint16_t item_len;
        if (!read_u16(list, list_len, lo, item_len)) return std::nullopt;
        if (lo + item_len > list_len) return std::nullopt;
