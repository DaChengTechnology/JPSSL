/**
 * sm4_gcm_avx2.cpp - SM4-GCM AVX2 backend.
 *
 * The CTR keystream phase uses the 8-way parallel SM4 engine from
 * sm4_avx2.cpp; H/J0 derivation and GHASH reuse the shared helpers in
 * sm4_gcm.cpp (PCLMULQDQ fast path with software fallback).
 *
 * Dispatched at runtime by sm4_gcm_dispatch.cpp via cpu_has_avx2().
 */
#include "sm4_gcm.hpp"
...
void sm4_gcm_encrypt_avx2(const sm4_ctx* ctx,
    ...
    sm4_ctr_avx2(ctx, ctr, plaintext.data(), ciphertext.data(), plaintext.size());
...
bool sm4_gcm_decrypt_avx2(const sm4_ctx* ctx,
    ...
    sm4_ctr_avx2(ctx, ctr, ciphertext.data(), plaintext.data(), ciphertext.size());
    return true;
}
...
void sm4_gcm_encrypt_avx2_inplace(const sm4_ctx* ctx,
    ...
    uint8_t ctr[16];
    sm4_gcm_ctr_start(j0, iv_len, ctr);
    sm4_ctr_avx2(ctx, ctr, buf, buf, data_len); // in-place safe
...
bool sm4_gcm_decrypt_avx2_inplace(const sm4_ctx* ctx,
    ...
    uint8_t ctr[16];
    sm4_gcm_ctr_start(j0, iv_len, ctr);
    sm4_ctr_avx2(ctx, ctr, buf, buf, data_len);
    return true;
}
