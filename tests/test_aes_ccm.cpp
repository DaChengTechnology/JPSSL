...
void test_roundtrip() {
    ...
    // AES-128, nonce=12, tag=16, with AAD
    {
        ...
        uint8_t tag[16];
        aes_ccm_encrypt(ctx, nonce, 12,
                        jpssl::span<const uint8_t>((const uint8_t*)pt_str, strlen(pt_str)),
                        jpssl::span<const uint8_t>((const uint8_t*)aad_str, strlen(aad_str)),
                        ct, tag, tag_len);
    ...
    // Non-block-aligned plaintext (17 bytes)
    {
        ...
        aes_ccm_encrypt(ctx, nonce, 13,
                        jpssl::span<const uint8_t>(pt, 17),
                        jpssl::span<const uint8_t>(),
                        ct, tag, tag_len);
...
void test_tamper_detection() {
    ...
    uint8_t tag[16];
    aes_ccm_encrypt(ctx, nonce, 12,
                    jpssl::span<const uint8_t>((const uint8_t*)pt, strlen(pt)),
    ...
    ok = aes_ccm_decrypt(ctx, nonce, 12,
                         jpssl::span<const uint8_t>(ct.data(), ct.size()),
                         jpssl::span<const uint8_t>((const uint8_t*)aad2.data(), aad2.size()),
                         tag, tag_len, pt_out);
