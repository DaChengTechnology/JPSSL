...
int main() {
    ...
    // Test 18: ECDHE shared secret (P-256/P-384) vs OpenSSL
    printf("=== Test 18: ECDH vs OpenSSL ===\n");
    {
        uint8_t pa[64], da[32], pb[64], db[32];
        jpssl::ecdsa_p256_keygen(pa, da);
        jpssl::ecdsa_p256_keygen(pb, db);
        uint8_t sa[32], sb[32];
        bool oka = jpssl::ecdsa_p256_ecdh(sa, da, pb);
        bool okb = jpssl::ecdsa_p256_ecdh(sb, db, pa);
        bool sym = oka && okb && memcmp(sa, sb, 32) == 0;
        uint8_t os[32];
        EC_GROUP* g256 = EC_GROUP_new_by_curve_name(NID_X9_62_prime256v1);
        EC_KEY* k256 = EC_KEY_new();
        EC_KEY_set_group(k256, g256);
        BIGNUM* d = BN_bin2bn(da, 32, nullptr);
        EC_KEY_set_private_key(k256, d);
        EC_POINT* peer = EC_POINT_new(g256);
        BIGNUM* px = BN_bin2bn(pb, 32, nullptr);
        BIGNUM* py = BN_bin2bn(pb + 32, 32, nullptr);
        BN_CTX* ctx = BN_CTX_new();
        EC_POINT_set_affine_coordinates(g256, peer, px, py, ctx);
        int n = ECDH_compute_key(os, 32, peer, k256, nullptr);
        bool ossl = sym && n == 32 && memcmp(sa, os, 32) == 0;
        printf("  P-256 ECDH symmetric: %s | vs OpenSSL: %s\n",
               sym ? "PASS" : "FAIL", ossl ? "PASS" : "FAIL");
        if (!ossl) { hex("our ", sa, 32); hex("ossl", os, 32); ++fails; }
        BN_free(d); BN_free(px); BN_free(py); BN_CTX_free(ctx);
        EC_POINT_free(peer); EC_KEY_free(k256); EC_GROUP_free(g256);

        uint8_t qa[96], ea[48], qb[96], eb[48];
        jpssl::ecdsa_p384_keygen(qa, ea);
        jpssl::ecdsa_p384_keygen(qb, eb);
        uint8_t ta[48], tb[48];
        bool oka2 = jpssl::ecdsa_p384_ecdh(ta, ea, qb);
        bool okb2 = jpssl::ecdsa_p384_ecdh(tb, eb, qa);
        bool sym2 = oka2 && okb2 && memcmp(ta, tb, 48) == 0;
        uint8_t os2[48];
        EC_GROUP* g384 = EC_GROUP_new_by_curve_name(NID_secp384r1);
        EC_KEY* k384 = EC_KEY_new();
        EC_KEY_set_group(k384, g384);
        BIGNUM* d2 = BN_bin2bn(ea, 48, nullptr);
        EC_KEY_set_private_key(k384, d2);
        EC_POINT* peer2 = EC_POINT_new(g384);
        BIGNUM* qx2 = BN_bin2bn(qb, 48, nullptr);
        BIGNUM* qy2 = BN_bin2bn(qb + 48, 48, nullptr);
        BN_CTX* ctx2 = BN_CTX_new();
        EC_POINT_set_affine_coordinates(g384, peer2, qx2, qy2, ctx2);
        int n2 = ECDH_compute_key(os2, 48, peer2, k384, nullptr);
        bool ossl2 = sym2 && n2 == 48 && memcmp(ta, os2, 48) == 0;
        printf("  P-384 ECDH symmetric: %s | vs OpenSSL: %s\n",
               sym2 ? "PASS" : "FAIL", ossl2 ? "PASS" : "FAIL");
        if (!ossl2) { hex("our ", ta, 48); hex("ossl", os2, 48); ++fails; }
        BN_free(d2); BN_free(qx2); BN_free(qy2); BN_CTX_free(ctx2);
