        ...
        for (size_t len : lens) {
            ...
            for (size_t tl : tag_lens) {
                for (bool use_aad : aad_cases) {
                    ...
                    const jpssl::span<const uint8_t> p_span(plain), a_span(aad);
                    ...
                    bool ok = sm4_ccm_decrypt_auto(
                        &ctx, nonce.data(), nl,
                        jpssl::span<const uint8_t>(ct_auto), a_span,
                        tag_auto, tl, pt_auto);
                    ...
                    if (!ct_auto.empty()) {
                        ...
                        ASSERT(!sm4_ccm_decrypt_auto(
                                   &ctx, nonce.data(), nl,
                                   jpssl::span<const uint8_t>(ct_bad), a_span,
                                   tag_auto, tl, pt_bad),
                    ...
                    if (has_gfni) {
                        ...
                        bool ok_gfni = jpssl::sm4_ccm_decrypt_gfni(
                            &ctx, nonce.data(), nl,
                            jpssl::span<const uint8_t>(ct_gfni), a_span,
                            tag_gfni, tl, pt_gfni);
...
static void test_large_inplace() {
    ...
    jpssl::span<const uint8_t> a_span(aad);

    // Scalar vector encrypt as the reference.
    std::vector<uint8_t> ct;
    uint8_t tag[16];
    sm4_ccm_encrypt(&ctx, nonce, 12, jpssl::span<const uint8_t>(plain),
                    a_span, ct, tag, 16);
